...
namespace UMS.Infrastructure.Services
{
	public class TokenService : BaseService<UMSDbContext, AppToken>, ITokenService
	{
		...
		public TokenService(IBaseRepository<UMSDbContext, AppToken> repo, IConfiguration configuration, IUserRepository userRepo, IUserRoleRepository userRoleRepo) : base(repo)
		{
		...
		public MessageObject<TokenDTO> LoginToken(string username, string password)
		{
			...
			using (var transaction = context.Database.BeginTransaction())
			{
				try
				{
					...
					user.Roles = _userRoleRepo.GetByConditionAsQueryable((e) => e.UserId == user.Id && e.Disabled == false).Include("AppRole").ToList();
					...
					message.Data = new()
					{
						...
						Roles = user.Roles.Select(e => e.AppRole?.Name ?? string.Empty).ToList(),
					};
