...
{
	public class AppProjectService : BaseService<UMSDbContext, AppProjectDto>, IAppProjectService
	{
		...
		public AppProjectService(IBaseRepository<UMSDbContext, AppProjectDto> baseRepo, HttpClient httpClient, ICacheService cacheService, ILogger<AppProjectService> logger) : base(baseRepo)
		{
		...
		public virtual async Task<MessageGetList<AppProjectDto>> GetAllDataProxy(QueryObject query, string additionalName = "")
		{
			string cacheKey = $"UMS-{_baseEndPoint}/page-with-disabled";
			if (string.IsNullOrEmpty(additionalName)) cacheKey += $"-{additionalName}";
			var cached = _cacheService.Get<MessageGetList<AppProjectDto>>(cacheKey);
			...
			try
			{
				...
				if (response.IsSuccessStatusCode)
				{
					...
					MessageGetList<AppProjectDto> messageGetList = Newtonsoft.Json.JsonConvert.DeserializeObject<MessageGetList<AppProjectDto>>(content) ?? new MessageGetList<AppProjectDto>();
					_cacheService.Set(cacheKey, messageGetList, TimeSpan.FromMinutes(defaultMinute));
					return messageGetList;
				}
				return new MessageGetList<AppProjectDto>();
			}
			catch (Exception ex)
			{
		...
		public virtual async Task<AppProjectDto?> GetByIdProxy(string Id, QueryObject query)
		{
			string cacheKey = $"UMS-{_baseEndPoint}/{Id}";
			var cached = _cacheService.Get<AppProjectDto>(cacheKey);
			if (cached != null) return cached;
			...
			try
			{
				...
				{
					var content = await response.Content.ReadAsStringAsync();
					AppProjectDto? data = Newtonsoft.Json.JsonConvert.DeserializeObject<AppProjectDto>(content);
